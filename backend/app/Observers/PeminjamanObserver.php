...
class PeminjamanObserver
{
    ...
    public function created(Peminjaman $peminjaman): void
    {
        ...
        $this->catatLog(
            "Peminjam ({$namaPeminjam}) membuat permohonan peminjaman baru (ID: #{$peminjaman->id})"
        );
    }
    ...
    public function updated(Peminjaman $peminjaman): void
    {
        if ($peminjaman->wasChanged('status')) {
            $this->catatLog(
                "Status peminjaman (ID: #{$peminjaman->id}) berubah menjadi: '{$peminjaman->status}'"
            );
        } else {
            if (!empty($peminjaman->getChanges())) {
                $this->catatLog(
                    "Memperbarui detail data peminjaman (ID: #{$peminjaman->id})"
                );
            }
        }
    }
    ...
    {
        $this->catatLog(
            "Membatalkan/menghapus permohonan peminjaman (ID: #{$peminjaman->id})"
        );
    }
}